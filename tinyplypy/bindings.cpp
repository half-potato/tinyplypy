...
pybind11::dict read_ply(const std::string &filename)
{
    ...
    for (auto &elm : file.get_elements())
    {
        ...
        for (auto &prop : elm.properties)
        {
            ...
            if (prop.isList)
            {
                // For fully general variable-length lists, you'd have to track the length for each row
                // and create a ragged array or store in an object array. For this example, we handle
                // fixed-size lists as indicated by prop.listCount (if > 0).
                if (prop.listCount > 0)
...
void write_ply(const std::string &filename, const pybind11::dict &pyDict, bool isBinary = true)
{
    ...
    for (auto item : pyDict)
    {
        ...
        for (auto propItem : properties)
        {
            ...
            {
                throw std::runtime_error("write_ply: Only 1D or 2D arrays are supported in this example");
            }
            ...
            else
            {
                ...
                // We'll guess a standard 1-byte list size type (Type::UINT8) for all fixed lists in this example.
                plyFile.add_properties_to_element(
                    elementName,
                    { propertyName },
                    plyType,
    ...
    plyFile.get_comments().push_back("Generated by tinyply + pybind11 binding example");
